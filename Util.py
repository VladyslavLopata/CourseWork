...
def try_all_in_radius(matrix, row, m):
            ...
            for k in range(m):
                for f in range(row, row+2):
                    if not (j == len(matrix[i]) or f == len(matrix[k])):
                        estimations.append([(i, j, k, f), radial_swap_calculate(
                            matrix, i, j, k, f)])  # memorize estimation
    ...
    matrix_swap(matrix, minimal_estimated[1][0], minimal_estimated[1]
                [1], minimal_estimated[1][2], minimal_estimated[1][3])  # swap
